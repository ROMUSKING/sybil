...
class DocumenterAgent extends agent_1.Agent {
...
Please create comprehensive documentation for this project.`;
    ...
    async generateReadme(projectName, description, blueprint, features) {
...
Generate a comprehensive README.md file for this project.`;
    ...
    async generateApiDocs(apiEndpoints) {
        ...
        const endpointsText = apiEndpoints.map(ep => `- ${ep.method.toUpperCase()} ${ep.path}: ${ep.description}`).join('\n');
...
- Authentication requirements`;
    ...
    async generateSetupGuide(dependencies, configSteps) {
...
- Verification steps`;
    ...
    async generateUserGuide(features, examples) {
        this.log('Generating user guide');
        const examplesText = examples.map(ex => `### ${ex.title}\n${ex.description}${ex.code ? `\n\`\`\`\n${ex.code}\n\`\`\`` : ''}`).join('\n\n');
...
- Common use cases`;
    ...
    async updateDocumentation(existingDocs, changes) {
...
//# sourceMappingURL=documenter_agent.js.map