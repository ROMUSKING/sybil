...
// Mock interfaces to avoid vscode dependency
interface UsageData {
    [provider: string]: {
        [model: string]: {
...
class TestModelManager {
    ...
    constructor(config: any, usageTracker: MockUsageTracker) {
        this.config = config;
    ...
    private getFriendlyNameFromModel(modelName: string): string {
        for (const [friendlyName, modelInfo] of Object.entries(this.config.models)) {
            if ((modelInfo as any).litellmModelName === modelName) {
                return friendlyName;
            }
        }
        return modelName;
...
const testConfig = localConfig || {
    api_keys: {
        openrouter: process.env.OPENROUTER_API_KEY || 'sk-or-v1-test-key', // Replace with real key or use config.local.json
        huggingface: process.env.HUGGINGFACE_API_KEY || 'hf_test_key', // Replace with real key or use config.local.json
...
async function testAPIConnections() {
    ...
    try {
        const response4 = await modelManager.sendRequest(
            ...
            ['or-deepseek-0528', 'or-gemini-flash', 'hf-qwen-coder']
        );
    ...
    console.log('\n=== Test 5: Code Generation Models ===');
    try {
        const response5 = await modelManager.sendRequest(
            'Create a React component for a todo list.',
            ['or-qwen-coder', 'featherless-gemma']
        );
